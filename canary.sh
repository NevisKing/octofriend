...
if [ -n "$ZSH_VERSION" ]; then
  _OCTOFIEND_DIR="${0:A:h}"
elif [ -n "$BASH_VERSION" ]; then
  _OCTOFIEND_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
else
...
function canary-octo() {
  (cd "$_OCTOFIEND_DIR" && npm run build) || return 1
  CANARY_OCTO=1 node "$_OCTOFIEND_DIR/dist/source/cli.js" "$@"
}
